import os
import time
#revChatGPT available on python >3.9
...
def get_answer(question, chatbot):
...
def collect_data(prompt,datasize = 30000):
  ...
  while True:
    if count > datasize:
      break
    try:
      result += extract_json(get_answer(prompt,chatbot))
      count+=1
      time.sleep(2)
    except SyntaxError:
      time.sleep(2)
...
if __name__ == "__main__":
  data_number = len(list(os.listdir(os.path.join(root,"processed_data"))))
  for i,filepath in enumerate(os.listdir(os.path.join(root,"chunking"))):
    if i >= data_number:
      with open(os.path.join(root,"chunking",filepath),"r") as fb:
          data = collect_data(create_prompt(fb.read()),10)
      with open(root + f"/processed_data/medical_full_{i}.json","w") as fc:
